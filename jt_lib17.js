function JT(id,w,h,fps,setupName,updateName,objName,mobileAudioSize,fullScreenBtn){
    ...
    this.loop={
            ...
            if(this.loaded==false){
              ...
              for (var im in this.context.assets.images) {
				...
				if(this.context.draw.cam[this.context.draw.currCam].active){
					if(this.context.draw.cam[this.context.draw.currCam].w<0.001){
						this.context.draw.cam[this.context.draw.currCam].w=0.001;
					}

					if(this.context.draw.cam[this.context.draw.currCam].h<0.001){
						this.context.draw.cam[this.context.draw.currCam].h=0.001;
					}
				}
				...
				if(this.context.draw.saveCpt>0){
					for(var i=0;i<this.context.draw.saveCpt;i++){
						this.context.draw.ctx.restore();
						if(i>9){
							break;
						}
					}
				...
				if(this.context.draw.cam[this.context.draw.currCam].active){
					...
					var camW=Math.abs(this.context.canvas.src.width/this.context.draw.cam[this.context.draw.currCam].w)
					var camH=Math.abs(this.context.canvas.src.height/this.context.draw.cam[this.context.draw.currCam].h)
					var camX=this.context.draw.cam[this.context.draw.currCam].x;
					var camY=this.context.draw.cam[this.context.draw.currCam].y;
				...
				if(this.context.draw.filtered){
					this.context.draw.resetFilter();
				}
				...
				if(document.activeElement.id!=this.context.canvas.id){
					...
					this.context.draw.camActive(before);
				}
				...
				if(this.debug){
					for(var i=0;i<this.debugs.length;i++){
						var d=this.debugs[i];
						var cam=this.context.draw.cam[this.context.draw.currCam].active;
						this.context.draw.cam[this.context.draw.currCam].active=false;
						if(d.type=="text"){
						...
						this.context.draw.cam[this.context.draw.currCam].active=cam;
        ...
        shake:function(force,duration,reduce){
            if(this.normalX==undefined){
                this.normalX=this.context.draw.cam[this.context.draw.currCam].x;
                this.normalY=this.context.draw.cam[this.context.draw.currCam].y;
            }
            if(force==undefined){
                force=this.fps/2;
            }
        ...
        shaking:function(){
			this.context.draw.cam[this.context.draw.currCam].x-=this.shakeObj.lastX;
			...
			this.context.draw.cam[this.context.draw.currCam].y-=this.shakeObj.lastY;
      ...
      camH=camH/posH;
		...
		restore:function(){
            ...
            if(this.rotated){
                ...
                this.rotatedCpt--;
                if(this.rotatedCpt<0){this.rotatedCpt=0;}
            }
			this.ctx.restore();
		},
                ...
                if(image.visible==false){
					...
					if(this.cam[this.currCam].active){
						if(rotation!=0 || this.rotated){
							var max=this.maxRotated(x-200,y-200,w+400,h+400);
			...
			if(p.anim===undefined && p.image===undefined){
				if(p.o===undefined){
					if(p.h==0 && p.hRate==0){
						...
						this.draw.rect(p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.c,p.r);
					}
				}else{
					if(p.h==0 && p.hRate==0){
						this.draw.circleB(p.x-p.w/2,p.y-p.w/2,p.w,p.c,p.o);
					}else{
						this.draw.rectB(p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.c,p.r,p.o);
					}
				}
			}else if(p.anim!==undefined){
				this.draw.anim(p.anim,p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.r);
			}else if(p.image!==undefined){
				this.draw.image(p.image,p.x-p.w/2,p.y-p.h/2,p.w,p.h,p.r);
			}
    ...
    this.mouse={
        ...
        check:function(x,y,w,h,press,cam,btn){
            ...
            if(cam==undefined){
                cam=this.draw.cam[this.draw.currCam].active;
            }
    ...
    this.createEventListeners=function(context) {
        context.canvas.src.addEventListener("mousemove", function(evt) {
			if(document.activeElement.id==context.canvas.id){
				...
				var camW=Math.abs(context.canvas.src.width/context.draw.cam[context.draw.currCam].w)
				var camH=Math.abs(context.canvas.src.height/context.draw.cam[context.draw.currCam].h)
				var camX=context.draw.cam[context.draw.currCam].x;
				var camY=context.draw.cam[context.draw.currCam].y;
        ...
        var posX=context.draw.cam[context.draw.currCam].pos.x;
        var posY=context.draw.cam[context.draw.currCam].pos.y;
        var posW=Math.abs(context.canvas.src.width/context.draw.cam[context.draw.currCam].pos.w)
        var posH=Math.abs(context.canvas.src.height/context.draw.cam[context.draw.currCam].pos.h)
        camW=camW/posW;
        camH=camH/posH;
    ...
    this.cam=function(num){
		if(num==undefined){num=this.draw.currCam;}
        return this.draw.cam[num];
    }

	this.camCurrent=function(num){
		return this.draw.currentCam(num);
	}
	...
	this.camActive=function(bool,num){
		return this.draw.camActive(bool,num);
	}

	this.inCanvas=function(x,y,w,h){
		if(typeof x=="number"){
			return this.draw.inCanvas(x,y,w,h);
		}else{
			return this.draw.inCanvas(x.x,x.y,x.w,x.h);
		}
	}

	this.inCanvasFull=function(x,y,w,h){
		if(typeof x=="number"){
			return this.draw.inCanvasFull(x,y,w,h);
		}else{
			return this.draw.inCanvasFull(x.x,x.y,x.w,x.h);
		}
	}

	this.inCam=function(x,y,w,h,num){
		if(typeof x=="number"){
			return this.draw.inCamera(x,y,w,h,num);
		}else{
			return this.draw.inCamera(x.x,x.y,x.w,x.h,num);
		}
	}

	this.inCamFull=function(x,y,w,h,num){
		if(typeof x=="number"){
			return this.draw.inCameraFull(x,y,w,h,num);
		}else{
			return this.draw.inCameraFull(x.x,x.y,x.w,x.h,num);
		}
	}

	this.xCam=function(x,num){
		return this.draw.xCam(x,num);
	}

	this.yCam=function(y,num){
		return this.draw.yCam(y,num);
	}

	this.camX=function(x,num){
		return this.draw.camX(x,num);
	}

	this.camY=function(y,num){
		return this.draw.camY(y,num);
	}
	...
	this.save=function(){
		return this.draw.save();
	}

	this.restore=function(){
		return this.draw.restore();
	}
    ...
    return this.draw.clip("shape",shape);
	}
  ...
  this.clips=function(shapes){
	...
	this.clipPoint=function(x,y){
		return this.draw.clip("rect",x,y,1,1,0);
	}

	this.clipLine=function(x1,y1,x2,y2,lineW,r){
		return this.draw.clip("line",x1,y1,x2,y2,r,lineW);
	}

	this.clipRect=function(x,y,w,h,r){
		return this.draw.clip("rect",x,y,w,h,r);
	}

	this.clipCircle=function(x,y,d){
		return this.draw.clip("circle",x+d/2,y+d/2,d/2,d/2,0);
	}

	this.clipEllipse=function(x,y,dX,dY,r){
		return this.draw.clip("ellipse",x+dX/2,y+dY/2,dX/2,dY/2,r);
	}
